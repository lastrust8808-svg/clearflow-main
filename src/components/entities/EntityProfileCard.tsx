...
export default function EntityProfileCard({
  ...
  const regenerateDispatchAssets = () => {
    ...
    setDraft((prev) => ({
      ...
      branding: {
        ...prev.branding,
        autoGenerateDispatchIdentity: true,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <label style={{ display: 'grid', gap: 6 }}>
          <span>Saved QR SVG</span>
          <textarea
            style={{ ...inputStyle, minHeight: 96, fontFamily: 'monospace', fontSize: 12 }}
            value={draft.branding?.entityQrSealSvg ?? ''}
            onChange={(event) =>
              setDraft((prev) => ({
                ...prev,
                branding: {
                  ...prev.branding,
                  entityQrSealSvg: event.target.value || undefined,
                },
              }))
            }
          />
        ...
        <label style={{ display: 'grid', gap: 6 }}>
          <span>Saved Mailing Barcode SVG</span>
          <textarea
            style={{ ...inputStyle, minHeight: 96, fontFamily: 'monospace', fontSize: 12 }}
            value={draft.branding?.entityMailingBarcodeSvg ?? ''}
            onChange={(event) =>
              setDraft((prev) => ({
                ...prev,
                branding: {
                  ...prev.branding,
                  entityMailingBarcodeSvg: event.target.value || undefined,
                },
              }))
            }
          />
        ...
        >
          <div style={{ display: 'grid', gap: 6 }}>
            ...
            <button
              ...
              onClick={() =>
                downloadSvgFile(
                  `${draft.displayName || draft.name}-dispatch-qr.svg`,
                  draft.branding?.entityQrSealSvg || '',
                )
              }
              style={{
                padding: '8px 10px',
          ...
          <div style={{ display: 'grid', gap: 6 }}>
            ...
            <button
              ...
              onClick={() =>
                downloadSvgFile(
                  `${draft.displayName || draft.name}-mailing-barcode.svg`,
                  draft.branding?.entityMailingBarcodeSvg || '',
                )
              }
              style={{
                padding: '8px 10px',
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        <button
          type="button"
          onClick={saveWithGeneratedAssets}
          style={{
            padding: '10px 14px',
            borderRadius: 12,
            border: '1px solid rgba(126, 242, 255, 0.28)',
