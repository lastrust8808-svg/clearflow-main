...
import TrustBuilderModal, { type TrustBuilderPayload } from '../ai-studio/TrustBuilderModal';
...
export default function AIStudioPage({ data, setData, onActiveEntityChange }: AIStudioPageProps) {
  const auth = useAuth();
  ...
  const persistGeneratedDocumentRecord = async (
    document: DocumentRecord,
    entityOverride?: CoreDataBundle['entities'][number]
    ...
    try {
      const entity = entityOverride || data.entities.find((item) => item.id === document.entityId);
...
`,
  ...
  const launchTrustBuilder = () => {
    setIsTrustBuilderOpen(true);
  };

  const handleTrustBuilderSubmit = async (payload: TrustBuilderPayload) => {
    const currentUser = auth.currentUser;
    const now = new Date();
    const isoNow = now.toISOString();
    const today = isoNow.slice(0, 10);
    const entityId = `ent-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const authorityId = `auth-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const trustDisplayName = payload.displayName.trim() || payload.trustName.trim();
    const trimmedTrustEmail = payload.trustEmail.trim();
    const storageEmail = trimmedTrustEmail || currentUser?.email || undefined;
    const storageMode =
      payload.storageMode || (currentUser?.email ? 'operator_google' : 'internal_only');
    const trustStyleLabelMap: Record<TrustBuilderPayload['trustStyle'], string> = {
      revocable_living: 'Revocable living trust',
      irrevocable: 'Irrevocable trust',
      land: 'Land trust',
      business: 'Business trust',
      charitable: 'Charitable trust',
      estate: 'Estate / testamentary trust',
      other: 'Other trust structure',
    };
    const dispatchIdentity = payload.generateDispatchIdentity
      ? buildEntityDispatchIdentity({
          entityId,
          entityName: trustDisplayName,
      ...
      : null;
    const governingDocId = payload.governingDocFile
      ? `doc-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
      : undefined;
    const trustEntity = {
      id: entityId,
      name: payload.trustName.trim(),
      displayName: trustDisplayName,
      type: 'trust' as const,
      primaryEmail: trimmedTrustEmail || currentUser?.email || undefined,
      jurisdiction: payload.governingState.trim() || undefined,
      country: payload.country.trim() || undefined,
      formationDate: payload.formationDate || today,
      status: 'active' as const,
      ownerDisplay: payload.grantorName.trim() || undefined,
      representativeName: payload.trusteeName.trim() || undefined,
      representativeRole: 'Trustee',
      authorityAttestedAt: isoNow,
      authorityAttestationStatement: `${payload.trusteeName.trim() || 'Acting trustee'} affirmed trustee authority to establish, administer, connect, and operate ${trustDisplayName} in ClearFlow.`,
      authorityProofDocumentId: governingDocId,
      authorityProofUploadedAt: payload.governingDocFile ? isoNow : undefined,
      authorityProofStatus: payload.governingDocFile ? ('review' as const) : ('missing' as const),
      authorityProofSummary: payload.governingDocFile
        ? 'Uploaded governing trust support is ready for authority review before live external transactions.'
        : 'Upload the trust agreement or certificate before using this trust for live bank, settlement, or reserve activity.',
      authorityProofNamedPartyNames: [
        payload.grantorName.trim(),
        payload.trusteeName.trim(),
        payload.successorTrusteeName.trim(),
      ].filter(Boolean),
      authorityProofRequiredPartyNames: [],
      authorityTransactionsPaused: !payload.governingDocFile,
      entityAccess: {
      ...
      branding: {
        accentColor: data.workspaceSettings.preferredAccentColor || '#36d7ff',
        documentLogoText: trustDisplayName,
        emailFromName: trustDisplayName,
        replyToEmail: trimmedTrustEmail || currentUser?.email || undefined,
        invoiceFooterNote: 'Trust administration and reserve records generated through ClearFlow.',
        sealValueEnabled: true,
        sealUnitValue: 1,
        sealValueCurrency: data.workspaceSettings.baseCurrency,
        sealTemplate: 'round' as const,
        sealPrimaryText: trustDisplayName,
        sealSecondaryText:
          payload.governingState.trim() || payload.country.trim() || 'ClearFlow Trust Seal',
        sealInkColor: data.workspaceSettings.preferredAccentColor || '#36d7ff',
        entitySealSvg: buildEntitySealDesign({
          entityName: trustDisplayName,
          jurisdiction: payload.governingState.trim() || payload.country.trim() || undefined,
          template: 'round',
          primaryText: trustDisplayName,
          secondaryText:
            payload.governingState.trim() || payload.country.trim() || 'ClearFlow Trust Seal',
          inkColor: data.workspaceSettings.preferredAccentColor || '#36d7ff',
        }),
      ...
      operationalDefaults: {
        baseCurrency: data.workspaceSettings.baseCurrency,
        fiscalYearStartMonth: 1,
        defaultSettlementPath: data.workspaceSettings.defaultSettlementPath,
        interEntitySettlementMode: data.workspaceSettings.defaultInterEntitySettlementMode,
        autoIssueVerificationTokens: data.workspaceSettings.autoIssueVerificationTokens,
        autoReconcileLedgerLinks: data.workspaceSettings.autoReconcileJournalEntries,
      },
    };
    ...
    const authorityRecord: AuthorityRecord = {
      id: authorityId,
      entityId,
      personName: payload.trusteeName.trim() || trustDisplayName,
      signerEmail: trimmedTrustEmail || currentUser?.email || undefined,
      recordType: 'trustee_authority',
      effectiveDate: payload.formationDate || today,
      clientAuthorizationStatus: 'active',
      approvalStatus: 'accepted',
      acceptedAt: isoNow,
      acceptedBy: payload.trusteeName.trim() || currentUser?.name || trustDisplayName,
      linkedDocumentIds: governingDocId ? [governingDocId] : undefined,
      notes: `Trust builder created a ${trustStyleLabelMap[payload.trustStyle]} profile for ${trustDisplayName}.`,
    };

    const persistedDocuments: DocumentRecord[] = [];

    if (payload.governingDocFile && governingDocId) {
      const fileMetadata = await saveDocumentFile(
        `doc-uploaded-${governingDocId}`,
        payload.governingDocFile
      );
      persistedDocuments.push({
        id: governingDocId,
        entityId,
        title: `${trustDisplayName} Governing Trust Record`,
        category: 'governing',
        date: today,
        status: 'final',
        outputStatus: 'review',
        linkedAuthorityRecordIds: [authorityId],
        summary: 'Uploaded trust agreement, certificate, or governing proof retained during trust setup.',
        fileName: fileMetadata.fileName,
        mimeType: fileMetadata.mimeType,
        sizeBytes: fileMetadata.sizeBytes,
        uploadedAt: fileMetadata.uploadedAt,
        sourceFileId: fileMetadata.sourceFileId,
        sourceRecordType: 'document',
        sourceRecordId: governingDocId,
        storageOwner: 'user_owned',
        retentionClass: 'authority',
        externalStorageTarget: 'google_drive',
        externalStorageStatus: 'ready',
        storageNotes:
          'Trust governing proof retained from the trust builder and ready for user-owned storage routing.',
      });
    }

    const builderPackets: DocumentRecord[] = [
      buildGeneratedDocument({
        entityId,
        title: `${trustDisplayName} Trust Builder Packet`,
        category: 'governing',
        summary:
          'Trust profile starter packet covering legal identity, trustee authority, funding intent, and operating next steps.',
        templateKey: 'formation_packet',
        body: `# Trust Builder Packet

## Trust Summary
- Trust name: ${trustDisplayName}
- Structure: ${trustStyleLabelMap[payload.trustStyle]}
- Governing state: ${payload.governingState.trim() || 'Not entered'}
- Formation date: ${payload.formationDate || today}
- Trust email: ${trimmedTrustEmail || 'Not entered'}

## Fiduciary Parties
- Grantor / settlor: ${payload.grantorName.trim() || 'Not entered'}
- Acting trustee: ${payload.trusteeName.trim() || 'Not entered'}
- Successor trustee: ${payload.successorTrusteeName.trim() || 'Not entered'}
- Beneficiary notes: ${payload.beneficiarySummary.trim() || 'Not entered'}

## Purpose And Funding
- Trust purpose: ${payload.trustPurpose.trim() || 'Not entered'}
- Initial funding plan: ${payload.fundingPlan.trim() || 'Not entered'}

## Immediate Next Steps
- Upload or review the governing trust agreement or certificate.
- Connect bank, reserve, or titled-property support only after authority proof is in place.
- Use the trust funding and administration packets to keep principal, income, reserve, and beneficiary activity cleanly separated.
`,
      }),
    ];

    if (payload.createTrusteePacket) {
      builderPackets.push(
        buildGeneratedDocument({
          entityId,
          title: `${trustDisplayName} Trustee Support Packet`,
          category: 'authority_record',
          summary:
            'Trustee packet covering authority acceptance, fiduciary duties, and administration controls.',
          body: `# Trustee Support Packet

Trust: ${trustDisplayName}

## Authority And Duty Notes
- Acting trustee: ${payload.trusteeName.trim() || 'Not entered'}
- Grantor / settlor: ${payload.grantorName.trim() || 'Not entered'}
- Governing state: ${payload.governingState.trim() || 'Not entered'}

## Operating Reminders
- Keep trust principal, income, reserve, and beneficiary distributions distinguishable.
- Retain supporting records for titled assets, cash funding, receipts, and reserve holdings.
- Do not release live payments or settlements until governing authority support is uploaded and reviewed.
`,
        })
      );
    }

    if (payload.createAdministrationPacket) {
      builderPackets.push(
        buildGeneratedDocument({
          entityId,
          title: `${trustDisplayName} Trust Administration Packet`,
          category: 'authority_record',
          summary:
...
`,
    ...
    if (payload.createFundingPacket) {
      builderPackets.push(
        buildGeneratedDocument({
          entityId,
          title: `${trustDisplayName} Trust Funding Packet`,
          category: 'financial',
          summary:
            'Trust funding packet covering initial funding, operating liquidity, titled assets, and fiduciary accounting posture.',
...
## Funding Objective
Confirm liquid funding, titled assets, reserve support, and governing terms before treating trust rights as operational cash.

## Initial Funding Plan
${payload.fundingPlan.trim() || '- No funding plan entered yet.'}
...
`,
    ...
    for (const packet of builderPackets) {
      persistedDocuments.push(await persistGeneratedDocumentRecord(packet, trustEntity));
    }

    const complianceTags: ComplianceTagRecord[] = [];
    if (!payload.governingDocFile) {
      complianceTags.push(
        buildComplianceTag({
          entityId,
          label: `${trustDisplayName} governing trust upload needed`,
          category: 'authority',
          status: 'review',
          dueDate: today,
          notes:
            'Trust builder created the profile without a governing trust agreement or certificate. Upload proof before live transactions or external settlement activity.',
        })
      );
    }
    complianceTags.push(
      buildComplianceTag({
        entityId,
        label: `${trustDisplayName} trust funding review`,
        category: 'entity',
        status: payload.fundingPlan.trim() ? 'review' : 'unknown',
        dueDate: today,
        notes: payload.fundingPlan.trim()
          ? 'Funding plan captured during trust setup. Confirm deposits, titled assets, and reserve support inside Accounting and Assets.'
          : 'No funding plan was entered during trust setup. Add funding sources before treating the trust as operationally funded.',
      })
    );

    setData((prev) => ({
      ...prev,
      entities: [trustEntity, ...prev.entities],
      authorityRecords: [authorityRecord, ...prev.authorityRecords],
      documents: [...persistedDocuments, ...prev.documents],
      complianceTags: [...complianceTags, ...prev.complianceTags],
    }));

    setReportEntityId(entityId);
    onActiveEntityChange?.(entityId);
    setIsTrustBuilderOpen(false);
    focusRoute('#entities');
  };
...
`,
  ...
  const studioTools: Array<{
    ...
    {
      title: 'Trust Creator & Builder',
      subtitle: 'Launch a trust board with authority, funding, and setup packets',
      detail: 'Create the trust profile, seed the trustee and funding paperwork, and carry the next-step requirements into the entity board in one move.',
      lane: 'trust',
      actionLabel: 'Build Trust',
      onAction: launchTrustBuilder,
    },
    {
  ...
  return (
    <div style={{ display: 'grid', gap: 20 }}>
      ...
      <TrustBuilderModal
        open={isTrustBuilderOpen}
        currentUserEmail={auth.currentUser?.email}
        currentUserName={auth.currentUser?.name}
        onClose={() => setIsTrustBuilderOpen(false)}
        onSubmit={handleTrustBuilderSubmit}
      />
